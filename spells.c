...
speldamage(x)
    ...
    if (x>=SPNUM) return(0);   /* no such spell */
    if (c[TIMESTOP])  { lprcat("  It didn't seem to work"); return(0); }  /* not if time stopped */
    clev = c[LEVEL];
    if ((rnd(23)==7) || (rnd(18) > c[INTELLIGENCE]))
        { lprcat("  It didn't work!");  return(0); }
    if (clev*3+2 < x)
    {
...
/* ----- LEVEL 1 SPELLS ----- */
        ...
        case 2: if (c[DEXCOUNT]==0) c[DEXTERITY]+=3; /* dexterity   */
                c[DEXCOUNT] += 400;     return(0);

        case 3: i=rnd(3)+1;
                p="  While the %s slept, you smashed it %d times";
            ws: direct(x,fullhit(i),p,i); /*    sleep   */  return(0);

        case 4: /*  charm monster   */  c[CHARMCOUNT] += c[CHARISMA]<<1;    return(0);
...
/* ----- LEVEL 2 SPELLS ----- */
        ...
        case 7: if (c[STRCOUNT]==0) c[STREXTRA]+=3; /*  strength    */
                c[STRCOUNT] += 150+rnd(100);    return(0);
        ...
        case 8: yl = playery-5;     /* enlightenment */
                ...
                for (i=yl; i<=yh; i++) /* enlightenment */
                    for (j=xl; j<=xh; j++)
                        know[j][i]=KNOWALL;
                draws(xl,xh+1,yl,yh+1); return(0);

        case 9: raisehp(20+(clev<<1));  return(0);  /* healing */

        case 10:    c[BLINDCOUNT]=0;    return(0); /* cure blindness   */

        case 11:    createmonster(makemonst(level+1)+8);  return(0);
        ...
        case 13:    /* if he has the amulet of invisibility then add more time */
                    ...
                    c[INVISIBILITY] += (j<<7)+12;   return(0);

/* ----- LEVEL 3 SPELLS ----- */

        case 14:    godirect(x,rnd(25+clev)+25+clev,"  The fireball hits the %s",40,'*'); return(0); /*    fireball */

        case 15:    godirect(x,rnd(25)+20+clev,"  Your cone of cold strikes the %s",60,'O');    /*  cold */
                    return(0);

        case 16:    dirpoly(x);  return(0);    /*  polymorph */

        case 17:    c[CANCELLATION]+= 5+clev;   return(0); /*  cancellation    */

        case 18:    c[HASTESELF]+= 7+clev;  return(0);  /* haste self  */
...
/* ----- LEVEL 4 SPELLS ----- */
        ...
        case 23:    i=min(c[HP]-1,c[HPMAX]/2);  /* drain life */
                    direct(x,i+i,"",0); c[HP] -= i;     return(0);
        ...
        case 26:    if (rnd(151)==63) { beep(); lprcat("\nYour heart stopped!\n"); nap(4000);  died(270); return(0); }
                    if (c[WISDOM]>rnd(10)+10) direct(x,2000,"  The %s's heart stopped",0); /* finger of death */
                    else lprcat("  It didn't work"); return(0);

/* ----- LEVEL 5 SPELLS ----- */

        case 27:    c[SCAREMONST] += rnd(10)+clev;  return(0);  /* scare monster */

        case 28:    c[HOLDMONST] += rnd(10)+clev;  return(0);  /* hold monster */

        case 29:    c[TIMESTOP] += rnd(20)+(clev<<1);  return(0);  /* time stop */

        case 30:    tdirect(x);  return(0);  /* teleport away */
...
/* ----- LEVEL 6 SPELLS ----- */
        ...
        case 34:    /* summon demon */
                    if (rnd(100) > 30) { direct(x,150,"  The demon strikes at the %s",0);  return(0); }
                    if (rnd(100) > 15) { lprcat("  Nothing seems to have happened");  return(0); }
                    lprcat("  The demon turned on you and vanished!"); beep();
                    i=rnd(40)+30;  lastnum=277;
                    losehp(i); /* must say killed by a demon */ return(0);

        case 35:    /* walk through walls */
                    c[WTW] += rnd(10)+5;    return(0);
        ...
        case 36:    /* alter reality */
            ...
            for (j=0; j<MAXY; j++)
                    ...
                    free((char*)save);   positionplayer();  return(0);
                    }
        ...
        default:    lprintf("  spell %d not available!",(long)x); beep();  return(0);
        };
    }
...
direct(spnum,dam,str,arg)
    ...
    int x,y;
    register int m;
    if (spnum<0 || spnum>=SPNUM || str==0) return(0); /* bad arguments */
    if (isconfuse()) return(0);
    dirsub(&x,&y);
    m = mitem[x][y];
    if (item[x][y]==OMIRROR)
        {
        ...
        fool:
            ...
            beep(); losehp(dam); return(0);
            }
        }
    if (m==0)
        {   lprcat("  There wasn't anything there!");   return(0);  }
    ifblind(x,y);
    if (nospell(spnum,m)) { lasthx=x;  lasthy=y; return(0); }
...
godirect(spnum,dam,str,delay,cshow)
    ...
    register int x,y,m;
    int dx,dy;
    if (spnum<0 || spnum>=SPNUM || str==0 || delay<0) return(0); /* bad args */
    if (isconfuse()) return(0);
    ...
    while (dam>0)
        ...
        if ((x==playerx) && (y==playery)) /* if energy hits player */
            ...
            lastnum=278;  losehp(dam);  return(0);
            }
...
tdirect(spnum)
    ...
    int x,y;
    register int m;
    if (spnum<0 || spnum>=SPNUM) return(0); /* bad args */
    if (isconfuse()) return(0);
    dirsub(&x,&y);
    if ((m=mitem[x][y])==0)
        {   lprcat("  There wasn't anything there!");   return(0);  }
...
omnidirect(spnum,dam,str)
    ...
    extern char lastmonst[] ;
    register int x,y,m;
    if (spnum<0 || spnum>=SPNUM || str==0) return(0); /* bad args */
...
dirpoly(spnum)
    ...
    int x,y,m;
    if (spnum<0 || spnum>=SPNUM) return(0); /* bad args */
    if (isconfuse()) return(0);    /* if he is confused, he can't aim his magic */
    dirsub(&x,&y);
    if (mitem[x][y]==0)
        {   lprcat("  There wasn't anything there!");   return(0);  }
    ifblind(x,y);
    if (nospell(spnum,mitem[x][y])) { lasthx=x;  lasthy=y; return(0); }
# ifdef MSDOS
    do {
        m = rnd(MAXMONST+7);
        mitem[x][y] = m;
