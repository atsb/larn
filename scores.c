...
showscores()
    {
    register int i,j;
    lflush();  lcreat((char*)0);  if (readboard()<0) return(0);
    i=winshou();    j=shou(0);
...
static newscore(score, whoo, whyded, winner)
    ...
    register int i;
    long taxes;
    if (readboard() < 0) return(0);    /*  do the scoreboard   */
    ...
    /* if he has a slot on the winning scoreboard update it if greater score */
        for (i=0; i<SCORESIZE; i++) if (winr[i].suid == userid)
                { new1sub(score,i,whoo,taxes); return(0); }
    ...
    /* if he has a slot on the scoreboard update it if greater score */
        for (i=0; i<SCORESIZE; i++) if (sco[i].suid == userid)
                { new2sub(score,i,whoo,whyded); return(0); }
    /* he had no entry. look for last entry and see if he has a greater score */
...
# endif
    if (c[LIFEPROT]>0) /* if life protection */
        ...
        lflush();  sleep(4);
        return(0); /* only case where died() returns */
        }
invalid:
