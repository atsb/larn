...
createitem(it,arg)
    ...
    {
    register int x,y,k,i;
    if (it >= MAXOBJ) return(0);   /* no such object */
    for (k=rnd(8), i= -8; i<0; i++,k++) /* choose direction, then try all */
        {
        if (k>8) k=1;   /* wraparound the diroff arrays */
        x = playerx + diroffx[k];       y = playery + diroffy[k];
        if (cgood(x,y,1,0)) /* if we can create here */
            {
            item[x][y] = it;  know[x][y]=0;  iarg[x][y]=arg;  return(0);
            }
        }
    }
...
hitmonster(x,y)
    ...
    if (c[TIMESTOP])  return(0);  /* not if time stopped */
    vxy(&x,&y); /* verify coordinates are within range */
    if ((monst = mitem[x][y]) == 0) return(0);
...
/*  spirit naga's and poltergeist's do nothing if scarab of negate spirit   */
    if (c[NEGATESPIRIT] || c[SPIRITPRO])  if ((mster ==POLTERGEIST) || (mster ==SPIRITNAGA))  return(0);
/*  if undead and cube of undead control    */
    if (c[CUBEofUNDEAD] || c[UNDEADPRO]) if ((mster ==VAMPIRE) || (mster ==WRAITH) || (mster ==ZOMBIE)) return(0);
    ...
    if (c[INVISIBILITY]) if (rnd(33)<20) 
        {
        lprintf("\nThe %s misses wildly",lastmonst);    return(0);
        }
    ...
    if (monster[mster].attack>0)
      if (((dam + bias + 8) > c[AC]) || (rnd((int)((c[AC]>0)?c[AC]:1))==1))
        { if (spattack(monster[mster].attack,x,y)) { lflushall(); return(0); }
...
dropsomething(monst)
    ...
    switch(monst)
        ...
        case PLATINUMDRAGON:  case GNOMEKING:  case REDDRAGON:
            something(level); return(0);

        case LEPRECHAUN: if (rnd(101)>=75) creategem();
                         if (rnd(5)==1) dropsomething(LEPRECHAUN);   return(0);
        }
    }
