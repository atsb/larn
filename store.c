...
dndstore()
  ...
  if (outstanding_taxes>0)
    ...
    drawscreen();  nosignal = 0; /* enable signals */ return(0);
    }
...
dnditem(i)
    ...
    {
    register int j,k;
    if (i >= MAXITM)  return(0);
    cursor( (j=(i&1)*40+1) , (k=((i%26)>>1)+5) );
    if (itm[i].qty == 0)  { lprintf("%39s","");  return(0); }
    lprintf("%c) ",(i%26)+'a');
...
oschool()
    ...
    while (1)
        ...
        if (i==12) { sch_hed();  continue; }
            ...
            { nosignal = 0; drawscreen();  /* enable signals */ return(0); }
        lprc(i);
