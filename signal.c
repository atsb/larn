...
static tstop() /* control Y */
    {
    if (nosignal)   return(0);  /* nothing if inhibited */
...
static sigill()  { sigpanic(SIGILL); }
static sigtrap() { sigpanic(SIGTRAP); }
static sigiot()  { sigpanic(SIGIOT); }
#ifdef SYSVSIGNALS
static sigemt()  { sigpanic(SIGEMT); }
#endif
static sigsegv() { sigpanic(SIGSEGV); }
static sigsys()  { sigpanic(SIGSYS); }
static sigpipe() { sigpanic(SIGPIPE); }
static sigterm() { sigpanic(SIGTERM); }
