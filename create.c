...
makemaze(k)
    ...
    if (k > 1 && (rnd(17)<=4 || k==MAXLEVEL-1 || k==MAXLEVEL+MAXVLEVEL-1))
        {
        if (cannedlevel(k));    return(0);     /* read maze from data file */
        }
    if (k==0)  tmp=0;  else tmp=OWALL;
    for (i=0; i<MAXY; i++)  for (j=0; j<MAXX; j++)  item[j][i]=tmp;
    if (k==0) return(0);       eat(1,1);
...
sethp(flg)
    ...
    if (level==0) { c[TELEFLAG]=0; return(0); } /* if teleported and found level 1 then know level we are on */
